...
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    ...
    FRESH_CLONE="$CONTRIB_APPIMAGE/fresh_clone/electrum-glc" && \
        rm -rf "$FRESH_CLONE" && \
        umask 0022 && \
...
docker run -it \
    ...
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/electrum-glc \
    --rm \
    --workdir /opt/electrum-glc/contrib/build-linux/appimage \
